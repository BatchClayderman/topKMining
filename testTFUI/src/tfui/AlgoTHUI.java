...
public class AlgoTHUI {
    ...
    Map<Integer, Map<Integer, Double>> mapLeafMAP = null; //�����յ�Map��
   ...
   //  private StringBuilder buffer = new StringBuilder(32);
    ...
    public void runAlgorithm(String input, String output, boolean eucsPrune) throws IOException {
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (int i = 0; i < items.length; ++i) {
                    ...
                    real1 = (real1 == null) ? util1 : util1 + real1;
                    real2 = (real2 == null) ? util2 : util2 + real2;
                    RIU.put(item, real1);
                    RIF.put(item, real2);
                }
            }
        ...
        // Raising threshold by real item utility    !!!!!!!!!!!!����Ҳ��Ҫ�޸ĵ�
        ...
        /*for (UtilityList listOfUtilityList : listOfUtilityLists) {
            System.out.println(listOfUtilityList.getItem());
        }*/
        //��listOfUtilityLists����item��TWUֵ������������ע���ǿ�ִ��ǰ�������
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (int i = 0; i < items.length; ++i) {
                    ...
                    if (mapItemToTWU.get(pair.item) >= minTopKValue) { //���������ֵ�������
                        revisedTransaction.add(pair);
                        remainingUtility += pair.utility;
                        remainingFrequency += pair.fre;
                        newTWU += alpha*pair.utility + beta* pair.fre; // NEW OPTIMIZATION ���������ֵ����Ȩ�ؽ�ȥ
                    }
                }
        ...
        thui(itemsetBuffer, 0, null, listOfUtilityLists); //�����itemsetBufferֻ�Ǹ�������
        checkMemory();
    ...
    public void updateLeafprune(int i, Pair pair, List<Pair> revisedTransaction, List<UtilityList> ULs) {
        ...
        for (int j = i - 1; j >= 0; --j) {
            ...
            cutil += alpha*pairAfter.utility + beta*pairAfter.fre; //��ʱ��Utility�Ѿ���frequency�����ȥ��
            twuItem += cutil;
            //twuItem2 += (int)cutil;
    ...
    private void thui(int[] prefix, int prefixLength, UtilityList pUL, List<UtilityList> ULs) throws IOException {
        for (int i = ULs.size() - 1; i >= 0; --i)
            if ((alpha*ULs.get(i).getIUtils() + beta*ULs.get(i).getFre()) >= minTopKValue) //�޸�(.L)
    ...
    private UtilityList construct(UtilityList P, UtilityList px, UtilityList py) {
        ...
        double totUtil = alpha * (px.sumIutils + px.sumRutils) + beta * (px.sumFre + px.sumRFre); //�޸�(.L)
        int ei = 0, ej = 0, Pi = -1;
