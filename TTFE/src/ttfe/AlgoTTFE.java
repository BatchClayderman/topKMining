...
public class AlgoTTFE
{
	...
	private LinkedHashMap<Integer, Double> RETF = new LinkedHashMap<>(); // RETF<event -> retf>: for each event -> retf = sum(each TF of events)
	...
	public class TreeNode implements Comparable<TreeNode>
	{
		...
		double atf = 0, etf = 0;
		public TreeNode(ArrayList<Integer> series, double atf, double etf)
		{
			this.series = series;
			this.atf = atf;
			this.etf = etf;
			...
			else
			{
				for (int i = 0; i < this.series.size(); ++i)
					if (treeNode.series.get(i) != this.series.get(i))
						return false;
				if (isAtf && treeNode.atf != this.atf)
					return false;
				if (isEtf && treeNode.etf != this.etf)
					return false;
				return true;
			}
		}
		...
		{
			if (this.etf < treeNode.etf)
				return 1;
			else if (this.etf > treeNode.etf)
				return -1;
			else
				return 0;
		}
	...
	public class Transaction extends AlgoTTFE // a line of the database
	{
		...
		public double getAtfPerTransaction(ArrayList<Integer> series)
		{
			double tranAtf = 0;
			for (int i = 0; i < series.size(); ++i)
		...
		public ArrayList<Integer> getMissingNodePerTransaction(ArrayList<Integer> series)
		{
			...
			if (this.getAtfPerTransaction(series) <= 0) // the current sequence is invalid
				return missingNodes;
			Set<Entry<Integer, TF>> set = this.events.entrySet();
			Iterator<Entry<Integer, TF>> iterator = set.iterator();
			while (iterator.hasNext())
	...
	private void computeRETF()
	{
		for (Transaction transaction : this.transactions)
		{
			...
			while (iterator.hasNext())
			{
				...
				this.RETF.put(key, (this.RETF.containsKey(key) ? this.RETF.get(key) : 0) + value.tf);
			}
		}
		
		if (this.isPrint)
		{
			printTitle(algoName + " - computeRETF()");
	...
	/* Sort RETF */
	private void sortRETF() // Selection sort (down)
	{
		ArrayList<Entry<Integer, Double>> entryList = new ArrayList<Entry<Integer, Double>>(this.RETF.entrySet());
		Collections.sort(entryList, new Comparator<Entry<Integer, Double>>() {
			@Override
			public int compare(Entry<Integer, Double> entry1, Entry<Integer, Double> entry2) {
			...
			for (ArrayList<Integer> series : this.LETF.combineSeries(this.LETF.series.length))
				for (Transaction trans : this.transactions)
				{
					...
					for (Transaction action : this.transactions) // sum up
						atf += action.getAtfPerTransaction(series);
					ArrayList<Integer> nodeEtf = trans.getMissingNodePerTransaction(series);
					double _etf = 0;
					for (int i = 0; i < nodeEtf.size(); ++i)
						_etf += this.RETF.get(nodeEtf.get(i));
					trans.addNode(series, atf, atf - _etf);
				}
	...
	public void pruneTree()
	{
		for (Transaction transaction : this.transactions)
			for (int i = transaction.treeNodes.size() - 1; i > -1; --i)
				if (transaction.treeNodes.get(i).atf + transaction.treeNodes.get(i).etf < this.delta)
	...
	public void getResults() throws IOException
	{
		...
		for (int i = 1; i < this.finalResults.size(); ++i)
		{
			...
			int j = i - 1;
			while (j >= 0 && this.finalResults.get(j).etf < current.etf)
			{
		...
		if (!this.finalResults.isEmpty())
			this.minTTFEValue = new Double(this.finalResults.get(this.finalResults.size() - 1).etf);
	...
	public boolean runAlgorithm(String inputFile, String outputFile) throws IOException
	{
		...
		this.computeRETF(); this.checkMemory();
		this.sortRETF(); this.checkMemory();
